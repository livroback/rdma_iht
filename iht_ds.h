...
void LinkedList::InitLinkedList(remote_ptr<LinkedList> p) {
    ...
    // Allocate memory for the head to no longer make it null
        p->head = pool_->Allocate<Node>();
        length = -1; 
        length++;  
        printf("LENGTH = %d\n", length); 
        head = p->head; // [ojr] just trying this .....
    
    //    [ojr] This line gives me a remote access error (Does allocate automatically make it a remote_nullptr ?) (it does not)  
